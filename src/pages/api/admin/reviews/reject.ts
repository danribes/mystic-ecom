...
import { sendReviewRejectionEmail } from '@/lib/email';
...
const putHandler: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (session.role !== 'admin') {
      throw new AuthorizationError('Only administrators can reject reviews');
    }
    ...
    logError(normalizedError, { context: 'reject_review_api' });
